...
The licence-exam modules (PPL, CPL, IR, ATPL) are **paused** and no longer build — see
`docs/APPS-FAMILY-ROADMAP.md`. Any Firebase app registrations already created for their
bundle ids are harmless; leave them.
...
> Earlier revisions named `com.flygaca.ppl` as the primary. PPL is a **paused** module, so
> the primary is now ELPT. Nothing is stranded: Sign in with Apple was removed from
> `apple/Apps/Shared/App.entitlements` in 2026-08 and the App IDs never carried the
> capability, so no user identifier was ever issued under the old primary.
