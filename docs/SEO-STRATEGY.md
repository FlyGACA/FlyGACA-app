...
> head manager) and pairs the *strategy* with the *technical work already shipped in this branch*
> (structured data, search-engine verification, an enriched sitemap) plus a scoped plan for static
> prerendering.
>
> **Positioning guardrail.** Fly GACA is **not affiliated with GACA**. It helps users *find and
> study* regulation and always cites the exact Part/section; it never replaces the official source.
...
<h2>2. The medical: GACA Class 1 (GACAR Part 67)</h2>
  <h3>Validity windows & where to renew</h3>     <!-- links /tools/medical-validity -->
<h2>3. Aviation English — SAELPT / ICAO Level 4+</h2>  <!-- links /guides/icao-english-saelpt -->
<h2>4. Theory & skill assessment</h2>            <!-- links /study/exam, /study/quiz -->
<h2>5. Document checklist & submission</h2>
<h2>6. Timeline, fees & common rejections</h2>
<h2>Frequently asked questions</h2>              <!-- FAQPage candidate (Phase 3) -->
...
Tools are JS calculators — their *value* must be visible to crawlers even before interaction:
...
### 3.3 Crawlability — static prerendering plan (scoped, not yet built)

**Problem.** The app is a client-rendered SPA: the served HTML is `<div id="root"></div>` and both
content *and* the per-route meta/JSON-LD appear only after hydration. Googlebot renders JS, but the
two-wave crawl is slower, less reliable for 360+ URLs, and weaker for Bing/social/AI crawlers.

**Recommended approach — post-build snapshot prerender (lowest blast radius):**

1. Add `scripts/prerender.mjs`, run as the build tail:
   `npm run build:sitemap && tsc -b && vite build && node scripts/prerender.mjs`.
2. It boots `vite preview`, drives a headless browser (Puppeteer; add as a `devDependency`) over a
   curated route list, waits until the managed meta + `script[data-managed-ld]` are present, then
   writes each route's fully-rendered HTML to `dist/<route>/index.html`.
3. **Route list** (reuse the enumeration already in `scripts/build-sitemap.mjs`):
   `/`, `/about`, `/pricing`, `/schools`, `/tools` + every `/tools/:id`, `/guides` + every
   `/guides/:slug`, `/library` + every Part. Skip private routes and the heavy `library-search`.
4. **Hosting:** update `firebase.json` rewrites so crawled paths serve their prerendered
   `index.html` (path-specific), keeping the SPA fallback for everything else. The client still
   hydrates and live-fetches `/data/*`, so the snapshot is *shell + above-fold*, never stale data.
