...
This runbook documents how to build, test, and manage the FlyGACA native iOS app family using automated npm scripts and the xcodebuild wrapper. The family is **ELPT** and **AIP**; both generate, build, and archive from the same shared shell. The licence-exam modules (PPL, CPL, IR, ATPL) are **paused** — see `docs/APPS-FAMILY-ROADMAP.md`.
...
AIP is the worked example — the smallest complete app
