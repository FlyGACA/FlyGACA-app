 ...
 * A mismatch makes the checkout callables resolve a nonexistent endpoint and
 * 404 in production, so this MUST ship in the same deploy that moves the
 * functions' region. The chat gateway (/api/chat) is reached by a hosting
 * fetch, not this region, so it is unaffected.
 */
