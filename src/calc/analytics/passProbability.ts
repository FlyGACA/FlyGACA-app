...
export const MODEL_COEFFICIENTS = {
  intercept: 0.0,
  wBestScore: 0.10,          // +1.0 logit per 10 points above 75
  wCoverage: 2.80,           // strong positive weight for bank coverage
  wTrend: 0.04,              // trajectory bonus
  wSrsMastery: 1.50,         // long-term memory retention
  wPacingPenalty: 0.40,      // penalty for rushing (<15s) or excessive drag (>120s)
  wVolatilityPenalty: 0.60,  // penalty for inconsistent erratic scoring
} as const;
...
export function calculatePassProbability(input: PassProbabilityInput): PassProbabilityResult {
  ...
  const xSrs = clamp(safeNumber(input.srsMasteryRatio, 0.5), 0, 1) - 0.70; // Centered at 70%
