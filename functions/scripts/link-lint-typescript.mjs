#!/usr/bin/env node
/**
 * Give typescript-eslint its own TypeScript 6 inside `functions/`, side by side
 * with the TypeScript 7 the build uses. Runs from this package's `postinstall`.
 *
 * `functions/` is its own npm package with its own CI job, and hits the same break
 * as the root: typescript-eslint@8 throws on load under TS 7.0, so `npm run lint`
 ...
 * script: `firebase deploy --only functions` uploads only this directory, so a
 * `../scripts/...` reference would be missing at deploy time and fail the install.
 *
 * Safe to run repeatedly, and non-fatal by design — a failure here must never break
 * a functions deploy. Under `--omit=dev` the alias is absent and this no-ops.
 */
