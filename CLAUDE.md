...
1. **The native SwiftUI family** (`apple/`) — one shared Swift package (`apple/FlyGACAKit`) plus
   one App Store app per study module, bundle ids `com.flygaca.<module>`, content generated by
   `scripts/build-ios-content.mjs` into `apple/Apps/<App>/Content/`. See `apple/ARCHITECTURE.md`.
...
> **The licence-exam native apps are paused** (2026-08-10). PPL, CPL, IR and ATPL were removed
> from `apple/` — folders, `project.yml` targets, the `APPS` maps in `build-ios-content.mjs` and
> `gen-app-icons.mjs`, npm scripts and CI matrices — and the `ppl-exam` **flavor** went with
> them. **The web packs are untouched**: still in `prepCatalog.ts`, still sold at
> `/study/packs/*`, still in `SELLABLE_PACK_IDS`. Don't re-add the apps, and don't "fix" a doc by
> restoring a six-app list. See `docs/APPS-FAMILY-ROADMAP.md`.
