...
- **`npm run verify` is the gate, and it is now effectively the *only* gate** — `.github/` is gone
  (see "Repo layout"), so no GitHub Actions run on a push. The one remaining automatic signal is
  Vercel's Git integration, which builds a preview per PR: it catches a broken `vite build` but
  nothing else, and it builds on **Linux**, so it is also where a case-only filename mistake
  surfaces. It chains
...
| Removed | Consequence |
| --- | --- |
| `.github/` | No GitHub Actions CI, no Firebase PR previews, no deploy workflow — only Vercel's preview build remains automatic. |
